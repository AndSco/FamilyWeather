...
const ForecastList = props => {
	...
	const [cities, setCities] = useState(localStorage.getItem("cities") ? JSON.parse(localStorage.getItem("cities")) : startingCities);
	...
	const removeCity = cityName => {
		...
		const filteredList = cities.filter(city => city.name !== cityName.toLowerCase());
	...
	const insertWeatherData = (city, data) => {
		const foundCity = cities.filter(entry => entry.name === city.name && entry.countryCode === city.countryCode)[0];
	...
	const sortByCold = () => {
		return cities.sort((a, b) => a.weatherData.list[0].main.temp - b.weatherData.list[0].main.temp);
	}

	const sortByHeat = () => {
		return cities.sort((a, b) => b.weatherData.list[0].main.temp - a.weatherData.list[0].main.temp);
	}


	const sortByWind = () => {
		return cities.sort((a, b) => b.weatherData.list[0].wind.speed - a.weatherData.list[0].wind.speed);
	}
	...
	const sortByRain = () => {   
		const citiesWithRain = cities
			...
			.sort((a, b) => b.weatherData.list[0].rain["3h"] - a.weatherData.list[0].rain["3h"]);
	...
	return (
		<div className="list">
			<div className={ openSearch ? "searching-div open-research" : "searching-div" }>
				<div className="search-icon" onClick={ () => setOpenSearch(!openSearch) }>
					<FontAwesomeIcon icon={ !openSearch ? "search" : "times-circle"} size="2x" />
				</div>	
				{ openSearch && <SearchBar addNewCity={addNewCity} closeSearchBar={closeSearchBar} handleSearching={handleSearching} cityList={cityList} /> }
			</div>	
			<Header sortByCold={sortByCold} sortByHeat={sortByHeat} sortByWind={sortByWind} sortByRain={sortByRain} />
